...
def evaluate_dataset(dataset, predictor, max_clicks, vis=True, vis_path='./experiments/vis_val/', **kwargs):
    all_ious = []
    ...
    for index in tqdm(range(len(dataset)), leave=False):
        ...
        _, sample_ious, _ = evaluate_sample(sample.image, sample.gt_mask, sample.init_mask, predictor,
                                            sample_id=index, max_clicks=max_clicks, vis=vis, save_dir=save_dir,
                                            index=index, **kwargs)
    ...
    with torch.no_grad():
        ...
        if vis and sample_id % 1000 == 0:
            if predictor.focus_roi is not None:
                ...
                out_image, focus_image = vis_result_refine(image, pred_mask, gt_mask, init_mask, iou, click_indx + 1,
                                                           clicks_list, focus_roi, global_roi, vis_pred, last_y, last_x,
                                                           focus_refined, focus_coarse)
                ...
                clicks_list = clicker.get_clicks()
                last_y, last_x = predictor.last_y, predictor.last_x
                out_image = vis_result_base(image, pred_mask, gt_mask, init_mask, iou, click_indx + 1, clicks_list,
                                            vis_pred, last_y, last_x)
                cv2.imwrite(save_dir + str(index) + '.png', out_image)
        return clicker.clicks_list, np.array(ious_list, dtype=np.float32), pred_probs


def vis_result_base(image, pred_mask, instances_mask, init_mask, iou, num_clicks, clicks_list, prev_prediction, last_y,
                    last_x):
    ...
    if h < w:
        out_image = cv2.hconcat(
            [image.astype(np.float32), fusion_init.astype(np.float32), fusion_pred.astype(np.float32),
             fusion_gt.astype(np.float32), fusion_prev.astype(np.float32)])
    else:
        out_image = cv2.hconcat(
            [image.astype(np.float32), fusion_init.astype(np.float32), fusion_pred.astype(np.float32),
             fusion_gt.astype(np.float32), fusion_prev.astype(np.float32)])
...
def vis_result_refine(image, pred_mask, instances_mask, init_mask, iou, num_clicks, clicks_list, focus_roi, global_roi,
                      prev_prediction, last_y, last_x, focus_refined, focus_coarse):
    image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
